...
locals {
  ...
  tags = {
    Environment = "Demo"
    Purpose     = "Simple AI Foundry with VNet and Search and CosmosDB and Capability Hosts and Managed Connections and Network Injections"
  }
}
...
resource "azurerm_subnet" "private_endpoints-foundry" {
...
resource "azurerm_subnet" "private_endpoints-agent" {
  name                 = "snet-private-endpoints-2"
  resource_group_name  = azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.vnet.name
...
resource "azurerm_subnet_network_security_group_association" "nsg_assoc" {
  subnet_id                 = azurerm_subnet.private_endpoints-foundry.id
...
resource "azapi_resource" "ai_foundry" {
  ...
  name                      = "foundry-${local.base_name}-${local.unique_suffix}"
  location                  = azurerm_resource_group.rg.location
  parent_id                 = azurerm_resource_group.rg.id
  ...
  body = {
    ...
    properties = {
      ...
      customSubDomainName = "foundry-${local.base_name}-${local.unique_suffix}"
      ...
      networkInjections = [
        {
          scenario                   = "agent"
          subnetArmId                = azurerm_subnet.private_endpoints-agent.id
  ...
  depends_on = [
    ...
    azurerm_subnet.private_endpoints-foundry,
...
resource "azurerm_private_endpoint" "ai_foundry_pe" {
  name                = "pe-foundry-${local.unique_suffix}"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  subnet_id           = azurerm_subnet.private_endpoints-foundry.id
  tags                = local.tags
...
resource "azapi_resource" "storage_connection" {
  ...
  body = {
    properties = {
      ...
      target   = "https://${azapi_resource.sa.name}.blob.core.windows.net"
      authType = "AAD"
      metadata = {
        ApiType    = "Azure"
        ResourceId = azapi_resource.sa.id
        location   = azurerm_resource_group.rg.location
      }
    }
  }
...
resource "azapi_resource" "log_analytics_connection" {
  type                      = "Microsoft.CognitiveServices/accounts/projects/connections@2025-06-01"
  ...
  body = {
    properties = {
      category = "AzureLogAnalytics"
      target   = azurerm_log_analytics_workspace.law.id
      authType = "AAD"
      metadata = {
        ApiType    = "Azure"
...
resource "azapi_resource" "application_insights_connection" {
  ...
  body = {
    properties = {
      category = "AzureApplicationInsights"
...
output "subnet_id" {
  value       = azurerm_subnet.private_endpoints-foundry.id
